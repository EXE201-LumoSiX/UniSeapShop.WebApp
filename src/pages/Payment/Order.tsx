...
import { ArrowLeft, CreditCard, Package, MapPin, User, Phone, Mail, CheckCircle, Clock, Truck } from 'lucide-react';
import { useSelector, useDispatch } from 'react-redux';
import { selectCartItems, selectCartTotal, selectCartItemsCount, clearCart } from '../../redux/feature/cartSlice';
...
const Order: React.FC = () => {
  ...
  const paymentFee = paymentMethods.find(method => method.id === selectedPaymentMethod)?.fee || 0;
  ...
  const validateForm = () => {
    ...
    else if (!/^[0-9]{10}$/.test(phone.trim())) errors.phone = 'Số điện thoại không hợp lệ';
    
    if (!email.trim()) errors.email = 'Vui lòng nhập email';
    else if (!/\S+@\S+\.\S+/.test(email.trim())) errors.email = 'Email không hợp lệ';
    
    if (!shippingAddress.trim()) errors.shippingAddress = 'Vui lòng nhập địa chỉ giao hàng';
    ...
    try {
      ...
      if (response.data && response.data.isSuccess) {
        ...
        if (selectedPaymentMethod === 'COD') {
          ...
          const paymentResponse = await api.post(`/api/payments/create-link/${orderId}`);
          ...
          if (paymentResponse.data && paymentResponse.data.isSuccess) {
            ...
            throw new Error(paymentResponse.data?.error || 'Không thể tạo đường dẫn thanh toán');
          }
        } else {
          throw new Error('Phương thức thanh toán không hợp lệ hoặc không nhận được mã đơn hàng');
        }
      } else {
                  ...
                  return (
                    <div key={method.id} className="relative">
                      ...
                      >
                        ...
                        <div className="ml-4">
                          {method.fee > 0 ? (
                            <span className="text-gray-600">+{method.fee.toLocaleString()} VNĐ</span>
                          ) : (
            ...
            <div className="bg-white rounded-xl shadow-md p-6 sticky top-8">
              ...
              <div className="space-y-4">
                ...
                <p className="text-center text-sm text-gray-600">
                  Bằng việc đặt hàng, bạn đồng ý với các <Link to="/terms" className="text-amber-800 hover:underline">điều khoản</Link> của chúng tôi
                </p>
              </div>
